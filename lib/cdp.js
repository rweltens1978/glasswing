...
export async function fetchJson(url, options = {}) {
  ...
  try {
    ...
    throw new CdpError(`Request failed for ${url}: ${error.message}`, {
      cause: error.cause?.message ?? error.cause ?? error.message,
      url
    });
  }
  if (response.statusCode < 200 || response.statusCode >= 300) {
    throw new CdpError(`HTTP ${response.statusCode} for ${url}`, response.body);
  }
...
async function request(url, options = {}) {
  ...
  return new Promise((resolve, reject) => {
    ...
    req.on("error", reject);
    req.setTimeout(Number(options.timeoutMs || 10000), () => {
      req.destroy(new CdpError(`Timed out requesting ${url}`));
    });
...
export async function activateTarget(port, id, host = "127.0.0.1") {
  await request(`http://${host}:${port}/json/activate/${id}`).catch(() => undefined);
}
