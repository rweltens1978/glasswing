...
export const defaults = {
  ...
  profilesRoot: process.env.GLASSWING_PROFILES_ROOT || `${homedir()}/Library/Application Support/Glasswing/Chrome Profiles`,
  profileName: process.env.GLASSWING_PROFILE_NAME || "default",
  profile: process.env.GLASSWING_PROFILE || "",
...
export function profilePathForName(name = defaults.profileName) {
  const safeName = String(name).trim();
  if (!safeName || safeName.includes("/") || safeName.includes("\\") || safeName === "." || safeName === "..") {
    throw new CdpError(`Invalid profile name: ${name}`);
  }
  return resolve(defaults.profilesRoot, safeName);
}

export function resolveProfile(args) {
  const profile = resolve(args.profile || defaults.profile || profilePathForName(args["profile-name"] || defaults.profileName));
  assertSafeProfile(profile);
  return profile;
}

function assertSafeProfile(profile) {
  const chromeDataRoot = resolve(homedir(), "Library/Application Support/Google/Chrome");
  if (profile === chromeDataRoot || profile.startsWith(`${chromeDataRoot}${sep}`)) {
    throw new CdpError(
      "Refusing to use Chrome's standard profile for CDP. Chrome 136+ ignores remote-debugging switches for the default data dir, and sharing a live profile can corrupt browser state. Use --profile-name work or --profile /path/to/non-default-profile."
    );
  }
}
...
export async function profilePath(args) {
  const name = args._[1] || args["profile-name"] || defaults.profileName;
  printJson({ name, path: profilePathForName(name) });
}
